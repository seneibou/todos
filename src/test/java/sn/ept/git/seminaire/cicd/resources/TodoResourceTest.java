...
class TodoResourceTest extends BasicResourceTest {
	...
	void findAll_shouldReturnTodos() throws Exception {
		dto = service.save(vm);
		mockMvc.perform(get(UrlMapping.Todo.ALL).accept(MediaType.APPLICATION_JSON)).andExpect(status().isOk())
				.andExpect(jsonPath("$.content", hasSize(1))).andExpect(jsonPath("$.content.[0].id").exists())
	...
	void findById_shouldReturnTodo() throws Exception {
		dto = service.save(vm);
		mockMvc.perform(get(UrlMapping.Todo.FIND_BY_ID, dto.getId()).accept(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk()).andExpect(jsonPath("$.id").exists())
				.andExpect(jsonPath("$.version").exists()).andExpect(jsonPath("$.enabled").exists())
				.andExpect(jsonPath("$.deleted").exists()).andExpect(jsonPath("$.title", is(dto.getTitle())))
	...
	void findById_withBadId_shouldReturnNotFound() throws Exception {
		mockMvc.perform(
				get(UrlMapping.Todo.FIND_BY_ID, UUID.randomUUID().toString()).accept(MediaType.APPLICATION_JSON))
				.andExpect(status().isNotFound());
	}

	@Test
	void add_shouldCreateTodo() throws Exception {
		mockMvc.perform(post(UrlMapping.Todo.ADD).contentType(MediaType.APPLICATION_JSON)
				.content(TestUtil.convertObjectToJsonBytes(vm))).andExpect(status().isCreated())
				.andExpect(jsonPath("$.id").exists()).andExpect(jsonPath("$.version").exists())
				.andExpect(jsonPath("$.enabled").exists()).andExpect(jsonPath("$.deleted").exists())
	...
	void add_withTitleMinLengthExceeded_shouldReturnBadRequest() throws Exception {
		...
		mockMvc.perform(post(UrlMapping.Todo.ADD).contentType(MediaType.APPLICATION_JSON)
				.content(TestUtil.convertObjectToJsonBytes(vm))).andExpect(status().isBadRequest());
	}
	...
	void update_shouldUpdateTodo() throws Exception {
		...
		mockMvc.perform(put(UrlMapping.Todo.UPDATE, dto.getId()).contentType(MediaType.APPLICATION_JSON)
	...
	void delete_shouldDeleteTodo() throws Exception {
		dto = service.save(vm);
		mockMvc.perform(delete(UrlMapping.Todo.DELETE, dto.getId()).contentType(MediaType.APPLICATION_JSON))
	...
	void delete_withBadId_shouldReturnNotFound() throws Exception {
		dto = service.save(vm);
		mockMvc.perform(
				delete(UrlMapping.Todo.DELETE, UUID.randomUUID().toString()).contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isNotFound());
	}
	...
	void complete_shouldCompleteTodo() throws Exception {
		dto = service.save(vm);
		mockMvc.perform(delete(UrlMapping.Todo.COMPLETE, dto.getId()).contentType(MediaType.APPLICATION_JSON))
	...
	void complete_withBadId_shouldReturnNotFound() throws Exception {
		dto = service.save(vm);
		mockMvc.perform(
				delete(UrlMapping.Todo.COMPLETE, UUID.randomUUID().toString()).contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isNotFound());
	}
}